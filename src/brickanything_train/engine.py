...
def do_train(
    ...
    if args.pretrained_weights is not None:
        sd = torch.load(args.pretrained_weights, map_location=torch.device("cpu"), weights_only=False)
    ...
    max_iters = args.max_epoch * len(dataloaders['train']) // args.gradient_accumulation_steps  #每个 epoch 有多少个 optimizer step
        ...
        for batch_idx, batch_data_label in enumerate(dataloaders['train']):
            ...
            with accelerator.accumulate(model):
                ...
                with accelerator.autocast():
                    ...
                    # print("autocast gpu dtype:", torch.get_autocast_gpu_dtype())
            ...
            if accelerator.sync_gradients:
                ...
                if curr_iter % args.log_every == 0:
                    ...
                    avg_loss_dict = {
                        f"train/{key}": torch.tensor(value, dtype=torch.float32).mean().item()
                        for key, value in loss_dict.items()
                    }

                    avg_loss_dict["train/learning_rate"] = curr_lr
                    avg_loss_dict["train/mem_mb"] = mem_mb
                    avg_loss_dict["train/epoch"] = curr_epoch

                    logger.info(
                        f"Epoch [{curr_epoch}/{args.max_epoch}]; "
                        f"Loss_dict [{avg_loss_dict}]; "
                    ...
                    accelerator.log(avg_loss_dict, step=curr_iter)
                    loss_dict = defaultdict(list)
                ...
                if do_eval_flag is True:
                    ...
                    if fixed_eval_loss_dict is None:
                        fixed_eval_loss_dict = {
                            key: float(value) for key, value in eval_loss_dict.items()
                        }
                    if accelerator.is_main_process:
                        print("Evaluation End, Begin Log!")
                        accelerator.log(fixed_eval_loss_dict, step=curr_iter+1)
