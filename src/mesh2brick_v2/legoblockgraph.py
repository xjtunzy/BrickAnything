...
# -------------------------
class LegoBlock:
    __slots__ = ("x","y","z","sx","sy","r","g","b","ignore_color","w")
    def __init__(self, x:int, y:int, z:int, sx:int=1, sy:int=1, r:int=255, g:int=255, b:int=255):
        self.x, self.y, self.z = x, y, z     # 左下角(或最小)坐标
        self.sx, self.sy = sx, sy            # 在同层的X/Y尺寸（单位：体素格）
        self.r, self.g, self.b = r, g, b     # 颜色（此实现仅占位）
        self.ignore_color = False
        self.w = 0                           # 组件权重（component_analysis中计算）
    ...
    # 合法乐高规格：1×{1,2,3,4,6,8} 与 2×{2,3,4,6,8}（顺序可交换）
    @staticmethod
...
class LegoBlockUtils:
    ...
    @staticmethod
    def are_connected(a:LegoBlock, b:LegoBlock)->bool:
        """ 垂直连接：z差1，且X/Y区间均有重叠 """
        if abs(a.z - b.z) != 1: return False
        return (LegoBlockUtils._overlap_1d(a.xspan, b.xspan) and
                LegoBlockUtils._overlap_1d(a.yspan, b.yspan))

    @staticmethod
    def are_adjacent(a:LegoBlock, b:LegoBlock)->bool:
        """ 同层相邻：z相等；X或Y方向相邻同时另一方向重叠 """
        if a.z != b.z: return False
        # x相邻 + y重叠
        x_touch = (a.xspan[1] == b.xspan[0]) or (b.xspan[1] == a.xspan[0])
        y_ovl   = LegoBlockUtils._overlap_1d(a.yspan, b.yspan)
        # y相邻 + x重叠
        y_touch = (a.yspan[1] == b.yspan[0]) or (b.yspan[1] == a.yspan[0])
        x_ovl   = LegoBlockUtils._overlap_1d(a.xspan, b.xspan)
        return (x_touch and y_ovl) or (y_touch and x_ovl)

    @staticmethod
    def can_merge(a:LegoBlock, b:LegoBlock)->bool:
        """ 判定两块是否能在同层合并成一个更大的矩形，并且尺寸为合法乐高规格 """
        if a.z != b.z: return False
        # 尝试在X方向拼接
        if a.yspan == b.yspan:
            xs = sorted([a.x, b.x])
            ...
            if sx is not None:
                if LegoBlock.has_valid_size(sx, a.sy):
                    return True
        # 尝试在Y方向拼接
        if a.xspan == b.xspan:
            ys = sorted([a.y, b.y])
            ...
            if sy is not None:
                if LegoBlock.has_valid_size(a.sx, sy):
                    return True
        return False

    @staticmethod
    def merge(a:LegoBlock, b:LegoBlock)->LegoBlock:
        """ 执行合并，返回新块（假设 can_merge=True） """
        assert a.z == b.z
        # X向拼接
        if a.yspan == b.yspan and (a.xspan[1] == b.xspan[0] or b.xspan[1] == a.xspan[0]):
            ...
            sx = a.sx + b.sx
            sy = a.sy
            return LegoBlock(x, y, a.z, sx, sy, a.r, a.g, a.b)
        # Y向拼接
...
class LegoBlockGraph:
    ...
    def __init__(self):
        ...
        self.graph: Dict[LegoBlock,List[LegoBlock]] = defaultdict(list)          # children (z+1)
        self.reverse_graph: Dict[LegoBlock,List[LegoBlock]] = defaultdict(list)  # parents  (z-1)
        self.horizontal_neighbours: Dict[LegoBlock,List[LegoBlock]] = defaultdict(list)
        self.levels: Dict[int,List[LegoBlock]] = defaultdict(list)
        self.visited: Dict[LegoBlock,int] = {}

    # ---------- 构建 ----------
    @staticmethod
    def from_numpy(occ: np.ndarray, color=(255,255,255)) -> "LegoBlockGraph":
        """
        occ: (Z, Y, X) 的布尔/0-1数组
        """
        g = LegoBlockGraph()
        ...
        for z in range(Z):
            for y in range(Y):
                xs = np.where(occ[z,y]>0)[0]
                for x in xs:
                    g.add_block(LegoBlock(x,y,z,1,1,*color))
        return g

    def get_neighbours_all(self, block:LegoBlock)->List[LegoBlock]:
        return list(self.graph[block]) + list(self.reverse_graph[block]) + list(self.horizontal_neighbours[block])

    def _filter_connected(self, blocks:List[LegoBlock], ref:LegoBlock)->List[LegoBlock]:
        return [b for b in blocks if LegoBlockUtils.are_connected(b, ref)]

    def _get_horizontal_neighbours_from_all(self, ref:LegoBlock)->List[LegoBlock]:
        return [b for b in self.blocks if b is not ref and LegoBlockUtils.are_adjacent(b, ref)]
    ...
    def remove_block(self, block:LegoBlock):
        ...
        for a in self.blocks:
            for b in self.horizontal_neighbours[a]:
                if (id(b), id(a)) in seen: 
                    continue
                if LegoBlockUtils.can_merge(a,b):
                    pairs.append((a,b))
                    seen.add((id(a), id(b)))
        return pairs
    ...
    def merge_to_maximal(self):
        ...
        while mergables:
            ...
            for nb in self.horizontal_neighbours[newb]:
                if LegoBlockUtils.can_merge(newb, nb):
    ...
    def _k_neighbourhood(self, root:LegoBlock, k:int)->Set[LegoBlock]:
    ...
    def _split_to_unit(self, block:LegoBlock)->List[LegoBlock]:
        res=[]
        for dx in range(block.sx):
            for dy in range(block.sy):
                res.append(LegoBlock(block.x+dx, block.y+dy, block.z, 1,1, block.r,block.g,block.b))
        return res

    def shuffle(self, w:LegoBlock)->"LegoBlockGraph":
        """ 复制图，取 w 的 K 邻域 + w 自身 -> 拆回 1×1 -> 再合并到极大，返回新图 """
        repl = LegoBlockGraph()
        # 深拷贝
        for b in self.blocks:
            b2 = LegoBlock(b.x,b.y,b.z,b.sx,b.sy,b.r,b.g,b.b)
            b2.ignore_color = b.ignore_color
            repl.add_block(b2)
        # 找到复制后的 w'
        #（简化：按坐标匹配）
        candidate = None
