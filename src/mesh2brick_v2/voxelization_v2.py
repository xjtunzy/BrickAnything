...
def voxelize_mesh(
    ...
    voxels_filled = vg.fill().matrix.astype(np.uint8)

    # 3. 抽壳处理 (Hollowing)
    if shell_thickness > 0:
        # 创建一个结构元素（3D球形或立方体，这里用立方体）
        struct = ndimage.generate_binary_structure(3, 1) 
        
        # 执行腐蚀操作：将物体向内收缩指定的厚度
        # iterations=3 对应论文保留 3 层厚度
        eroded_voxels = ndimage.binary_erosion(voxels_filled, structure=struct, iterations=shell_thickness)
        
        # 异或/减法操作：原始物体 减去 缩小后的物体 = 壳
        voxels = np.logical_xor(voxels_filled, eroded_voxels).astype(np.uint8)
    else:
        voxels = voxels_filled
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default='/mnt/data/yanfeng/n_project/brick_gen/BrickAnything/src/mesh2brick_v2/data/tucan.obj', help='输入的 mesh 文件路径 (.ply / .obj)')
